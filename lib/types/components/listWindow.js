 ...
 */
export function listWindow(length, focusIndex, maxVisible) {
    const count = Math.max(1, maxVisible);
    const start = Math.max(0, Math.min(focusIndex - Math.floor(count / 2), length - count));
    return { start, end: Math.min(start + count, length) };
}
